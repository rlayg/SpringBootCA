...
// controller 대행 / chatView.jsp가 Client
@Component
public class SocketHandler extends TextWebSocketHandler {
	...
	// 메소드는 메시지를 수신하면 실행
	@SuppressWarnings("unchecked")	// 노란경고 없애준데
	@Override
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		switch (msgType) {
			...
			// sessionUserMap에 sessionId와 userName 등록
				...
				System.out.println("== 2. sessionId : sessionUserMap.sessionId  ==");
				System.out.println("== 3. userSave  : sessionUserMap.yserName   ==");
	...
	public void afterConnectionEstablished(WebSocketSession session) throws Exception {
		...
		// 웹소켓  연결이 되면 동작
		super.afterConnectionEstablished(session);	// call by ref
		// 연결 소켓을 mAP에 등록
		sessionMap.put(session.getId(), session);	// session이 콜바이 레퍼런스라 여기 안에 있는 session.getId() 꺼내다 쓰는거
